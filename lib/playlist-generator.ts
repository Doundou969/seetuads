 ...
 * A, B, C, A, B
 */
function buildCampaignOccurrences(
  campaign: EligibleCampaign
): GeneratedItem[] {
  const media = [...campaign.campaignMedia].sort(
    (a, b) =>
  ...
  if (media.length === 0) {
    throw new Error(
      `La campagne ${campaign.id} ne contient aucun mÃ©dia.`
    );
  }
  ...
  for (const campaignMedia of media) {
    if (campaignMedia.media.status !== "APPROVED") {
      throw new Error(
        `Le mÃ©dia ${campaignMedia.mediaId} de la campagne ${campaign.id} n'est pas approuvÃ©.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `DurÃ©e incohÃ©rente pour le mÃ©dia ${campaignMedia.mediaId} de la campagne ${campaign.id}.`
      );
    }
  }

  const occurrences: GeneratedItem[] = [];

  for (
    let index = 0;
    index < campaign.frequencyPerLoop;
    index++
  ) {
    const campaignMedia =
 ...
 */
function distributeOccurrences(
  ...
  const queues = campaigns
    ...
    .sort(
      (a, b) =>
        a.campaign.createdAt.getTime() -
          b.campaign.createdAt.getTime() ||
        a.campaign.id.localeCompare(
          b.campaign.id
        )
    );
  ...
  while (queues.some((queue) => queue.items.length > 0)) {
    ...
    available.sort((a, b) => {
      const aDifferent =
        a.campaign.id === lastCampaignId ? 1 : 0;
      const bDifferent =
        b.campaign.id === lastCampaignId ? 1 : 0;
    ...
    if (!item) {
      throw new Error(
        "Erreur interne lors de la gÃ©nÃ©ration de la playlist."
      );
    }
...
export async function regenerate(
  ...
  if (!screenId) {
    throw new Error(
      "Un Ã©cran est requis pour rÃ©gÃ©nÃ©rer une playlist."
    );
  }
  ...
  if (!screen) {
    throw new Error(
      `Ã‰cran introuvable : ${screenId}.`
    );
  }

  if (screen.inventoryLoopSeconds <= 0) {
    throw new Error(
      `La capacitÃ© de boucle de l'Ã©cran ${screenId} est invalide.`
    );
  }
  ...
  const campaignsById =
    new Map<string, EligibleCampaign>();
  ...
  for (const reservation of reservations) {
    ...
    if (campaign.campaignScreens.length === 0) {
      throw new Error(
        `La rÃ©servation ${reservation.id} ne correspond pas Ã  un CampaignScreen actif pour l'Ã©cran ${screenId}.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `La durÃ©e du spot de la campagne ${campaign.id} est invalide.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `La frÃ©quence de la campagne ${campaign.id} est invalide.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Allocation incohÃ©rente pour la campagne ${campaign.id} : rÃ©servation ${reservation.reservedSeconds}s, attendu ${expectedReservedSeconds}s.`
      );
    }
    ...
    if (campaignsById.has(campaign.id)) {
      throw new Error(
        `Plusieurs rÃ©servations CONFIRMED alimentent la campagne ${campaign.id} sur l'Ã©cran ${screenId}.`
      );
    }

    campaignsById.set(
      campaign.id,
      campaign as EligibleCampaign
    );
  }
  ...
  const generatedItems =
    distributeOccurrences(campaigns);
  ...
  if (generatedSeconds !== reservedSeconds) {
    throw new Error(
      `La durÃ©e gÃ©nÃ©rÃ©e (${generatedSeconds}s) ne correspond pas Ã  la durÃ©e rÃ©servÃ©e (${reservedSeconds}s) sur l'Ã©cran ${screenId}.`
    );
  }
  ...
  if (
    ...
    throw new Error(
      `La playlist de l'Ã©cran ${screenId} dÃ©passe sa capacitÃ© : ${generatedSeconds}s gÃ©nÃ©rÃ©es pour ${screen.inventoryLoopSeconds}s disponibles.`
    );
  }
  /*
  ...
  const hasManualItems =
    ...
    currentActivePlaylist.items.every(
      (item) => item.campaignId === null
    );
  ...
  if (generatedItems.length === 0) {
    if (hasManualItems) {
      console.log(
        `Aucune campagne commerciale Ã©ligible pour l'Ã©cran ${screenId}. ` +
